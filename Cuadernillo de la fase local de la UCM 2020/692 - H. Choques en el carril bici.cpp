...
bool solve() {
  ...
  for (int i = 1; i<n; i++){
    ...
    if (v[i].second-v[i-1].second != 0){
        caso = (v[i-1].first-v[i].first)/(v[i].second-v[i-1].second);
        if (((v[i-1].first-v[i].first) > 0 && (v[i].second-v[i-1].second) > 0) || ((v[i-1].first-v[i].first) < 0 && (v[i].second-v[i-1].second) < 0)){
            sol = min(sol, caso);
        }
    }
  }
